...
from admin_panel.models import Wings_Model,Student_info_Model
...
class Programmes_Registration_Model(models.Model):
    ...
    Organizer_Wing = models.ForeignKey( 
        Wings_Model,on_delete=models.CASCADE
        
        ) #add the wing name from wing model
    
    Programme_Img = models.ImageField(upload_to='programme/', null=True, blank=True)
    created_by = models.ForeignKey(Wings_Model,on_delete=models.CASCADE)# who created it 
...
#     wing_Name = models.CharField(Wings_Model,on_delete= models.CASCADE)
